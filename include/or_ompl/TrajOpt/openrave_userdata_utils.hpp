...
void RemoveUserData(T& body, const std::string& key) {
  ...
  if (UserMap* um = dynamic_cast<UserMap*>(ud.get())) {
    if (um->find(key) == um->end()) RAVELOG_WARN("tried to erase key %s but it's not in the userdata map!", key.c_str());
    (*um).erase(key);
  }
  else {
    RAVELOG_ERROR("body %s has no userdata map", body.GetName().c_str());
  }
}
