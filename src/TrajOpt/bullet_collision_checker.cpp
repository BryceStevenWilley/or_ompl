...
COWPtr CollisionObjectFromLink(OR::KinBody::LinkPtr link, bool useTrimesh) {
  ...
  if ((link->GetGeometries().size() == 1) && isIdentity(link->GetGeometry(0)->GetTransform())) {
    RAVELOG_DEBUG("using identity for %s", link->GetName().c_str());
    ...
    RAVELOG_DEBUG("NOT using identity for %s", link->GetName().c_str());
...
void RenderCollisionShape(btCollisionShape* shape, const btTransform& tf,
  ...
  case CONVEX_HULL_SHAPE_PROXYTYPE: {
    ...
    if ( it != gHullCache.end() )
      ...
      if (hl.CreateConvexHull(hd, hr) == QE_FAIL) {
        RAVELOG_ERROR("convex hull computation failed on shape with %i vertices", convex->getNumPoints());
        hr.mNumFaces = 0;
      }
      else {
      }
  ...
  default:
    RAVELOG_INFO("not rendering shape of type %i", shape->getShapeType());
    break;
  }
}
...
public:
  ...
  virtual void ContinuousCheckTrajectory(const DblMatrix& traj, Configuration& rad, vector<Collision>&);
...
struct CollisionCollector : public btCollisionWorld::ContactResultCallback {
  ...
  virtual btScalar addSingleResult(btManifoldPoint& cp,
    ...
    RAVELOG_DEBUG("CollisionCollector: adding collision %s-%s (%.4f)", linkA->GetName().c_str(), linkB->GetName().c_str(), cp.m_distance1);
    return 1;
  }
...
void BulletCollisionChecker::SetContactDistance(float dist) {
  RAVELOG_DEBUG("setting contact distance to %.2f", dist);
...
void BulletCollisionChecker::AllVsAll(vector<Collision>& collisions) {
  RAVELOG_WARN("WARNING: AllVsAll seems to be broken! (since a8f8da01)");
  UpdateBulletFromRave();
  RAVELOG_DEBUG("AllVsAll");
  m_world->performDiscreteCollisionDetection();
  int numManifolds = m_dispatcher->getNumManifolds();
  RAVELOG_DEBUG("number of manifolds: %i", numManifolds);
  for (int i = 0; i < numManifolds; ++i) {
    btPersistentManifold* contactManifold = m_dispatcher->getManifoldByIndexInternal(i);
    int numContacts = contactManifold->getNumContacts();
    RAVELOG_DEBUG("number of contacts in manifold %i: %i", i, numContacts);
...
//      RAVELOG_DEBUG("local pts: %s\n",ss.str().c_str());
      ...
      if (CanCollide(objA, objB)) {
        ...
        RAVELOG_DEBUG("ignoring collision between %s and %s", bodyA->GetName().c_str(), bodyB->GetName().c_str());
        assert(0 && "this shouldn't happen because we're filtering at narrowphase");
      }
      RAVELOG_DEBUG("%s - %s collided", bodyA->GetName().c_str(), bodyB->GetName().c_str());
    }
  ...
  for (const OR::KinBody::LinkPtr& link : links) {
    if (link->GetGeometries().size() > 0) {
      ...
      if (new_cow) {
        ...
        RAVELOG_DEBUG("added collision object for  link %s", link->GetName().c_str());
        cd->links.push_back(link.get());
        cd->cows.push_back(new_cow);
      }
      else {
        RAVELOG_WARN("ignoring link %s", link->GetName().c_str());
      }
    }
  }
...
void BulletCollisionChecker::RemoveKinBody(const OR::KinBodyPtr& body) {
  RAVELOG_DEBUG("removing %s", body->GetName().c_str());
...
void BulletCollisionChecker::UpdateBulletFromRave() {
  ...
  btCollisionObjectArray& objs = m_world->getCollisionObjectArray();
  RAVELOG_DEBUG("%i objects in bullet world", objs.size());
  for (int i=0; i < objs.size(); ++i) {
    CollisionObjectWrapper* cow = static_cast<CollisionObjectWrapper*>(objs[i]);
...
void BulletCollisionChecker::ContinuousCheckTrajectory(const DblMatrix& traj, Configuration& rad, vector<Collision>& collisions) {
  UpdateBulletFromRave();
  m_world->updateAabbs();
  ...
  for (int iStep=0; iStep < traj.rows(); ++iStep) {
    const Eigen::VectorXd x = traj.row(iStep);
    std::vector<double> xVals(x.data(), x.data() + x.size());
    rad.SetDOFValues(xVals);
...
void BulletCollisionChecker::CastVsAll(Configuration& rad, const vector<KinBody::LinkPtr>& links,
  ...
  RAVELOG_DEBUG("CastVsAll checked %li links and found %li collisions", links.size(), collisions.size());
}
