...
void RobotAndDOF::SetRobotActiveDOFs()  {
  ...
  std::vector<int> current_active = robot1->GetActiveDOFIndices();
...
void RobotAndDOF::GetAffectedLinks(std::vector<KinBody::LinkPtr>& links, bool only_with_geom, std::vector<int>& link_inds) {
  links.clear();
  link_inds.clear();
  for (const KinBody::LinkPtr& link : GetRobot()->GetLinks()) {
...
std::vector<KinBodyPtr> RobotAndDOF::GetBodies() {
  ...
  return std::vector<KinBodyPtr> (bodies.begin(), bodies.end());
} 
...
DblVec RobotAndDOF::RandomDOFValues() {
  ...
  for (int i=0; i < ndof; ++i) {
    ...
    out[i] = lower[i] + ((float) rand() / (float) RAND_MAX) * (upper[i] - lower[i]);
  }
  return out;
}
